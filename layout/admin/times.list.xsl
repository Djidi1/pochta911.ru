	...
	<xsl:template match="container[@module = 'timeslist']">
		...
		<form method="post" class="time_check_list">
			...
			<ul class="list-group list-group-hover list-group-striped">
				<li class="list-group-item">Заказ на завтра с доставкой до <input name="period_tomarrow[from]" class="form-control" type="number" value="{times/period_tomarrow/from}"/> ч. можно оставлять до <input name="period_tomarrow[to]" class="form-control" type="number" value="{times/period_tomarrow/to}"/> ч.</li>

				<li class="list-group-item">Заказ на сегодня с доставкой до <input name="period_today[from]" class="form-control" type="number" value="{times/period_today/from}"/> ч. можно оставлять с <input name="period_today[to]" class="form-control" type="number" value="{times/period_today/to}"/> ч.</li>

				<li class="list-group-item">Заказ с готовностью с <input name="ready_1[from]" class="form-control" type="number" value="{times/ready_1/from}"/> до <input name="ready_1[to]" class="form-control" type="number" value="{times/ready_1/to}"/> можно доставить в течении <input name="ready_1[period]" class="form-control" type="number" value="{times/ready_1/period}"/> ч.</li>
				...
				<li class="list-group-item">Период забора не менее <input name="period_from[period]" class="form-control" type="number" value="{times/period_from/period}"/> минут.</li>
