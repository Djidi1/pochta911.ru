...
// If you're serving to IE over SSL, then the following may be needed
	...
	public function getRoutes($order_id) {
		$sql = 'SELECT r.id id_route, `to`,to_region,to_AOGUID,to_house,to_appart,
					  to_fio,to_phone,cost_route,cost_tovar,cost_car,
					  `to_time`,`to_time_end`,r.`to_comment`, s.status, s.id status_id, r.pay_type, 
					   r.goods_type, r.goods_val, r.target
	...
	public function getOrderRoute($order_route_id) {
	    $sql = "SELECT id id_route, id_order, `to`, to_house, to_appart, to_fio, to_phone, 
                      cost_route,cost_tovar,cost_car, to_time, `to_comment`, id_status, dk 
                FROM orders_routes
    ...
    public function getOrderInfo($order_id) {
        $sql = 'SELECT o.id,
                       CONCAT(`from`,\', \',from_house,\', \',from_appart) from_addr,
					   from_comment,
					   from_fio, from_phone,
                       u.inkass_proc,
                       o.id_car,
    ...
    public function getOrderRoutesInfo($order_id) {
        $sql = 'SELECT r.id,
                       ...
                       r.to_time,
                       r.to_time_end,
                       CONCAT(r.`to`,\', \',r.`to_house`,\', \',r.`to_appart`) to_addr,
                       r.to_fio,
                       r.to_phone,
                       r.pay_type,
					   r.cost_route,
	...
	public function getOrder($order_id) {
		$sql = 'SELECT o.id, id_user, date, 
                      `from`, from_region, from_AOGUID, from_house, from_appart, from_fio, from_phone, 
                      time_ready_from, time_ready_end, from_comment, id_car, car_accept, 
                      o.isBan, o.dk,
					   u.name,
					   u.title,
					   u.phone,
...
*/
	public function getLogistList($from, $to, $user_id = 0) {
		$sql = 'SELECT o.id, 
                       o.`from` AS address,
                       o.from_comment AS addr_comment, 
                       u.name,
                       u.title, 
                       o.date, 
                       o.time_ready_from,
	...
	public function orderInsert($id_user, $params) {
        ...
        $from = $params['from'][0];
        $from_region = $params['from_region'][0];
        $from_AOGUID = $params['from_AOGUID'][0];
        $from_house = $params['from_house'][0];
        $from_appart = $params['from_appart'][0];
        $from_fio = $params['from_fio'][0];
        $from_phone = $params['from_phone'][0];
        $time_ready_from = $params['time_ready_from'][0];
        $time_ready_end = $params['time_ready_end'][0];
        $from_comment = $params['from_comment'][0];

		$sql = "
		INSERT INTO orders (id_user,date,`from`,from_region,from_AOGUID,from_house,from_appart,from_fio,from_phone,
		                    time_ready_from,time_ready_end,from_comment,dk)
		VALUES ($id_user,'$date','$from','$from_region','$from_AOGUID','$from_house','$from_appart','$from_fio','$from_phone',
		        '$time_ready_from','$time_ready_end','$from_comment',NOW());
		";
	...
	public function orderUpdate($params) {
        ...
        $time_ready_end = $params['time_ready_end'][0];
        $from_comment = $params['from_comment'][0];
        $car_courier = $params['car_courier'];
		$sql = "
		UPDATE orders SET 
		 id_user = '$id_user' -- id_user - VARCHAR(255)
         ,date = '".$this->dmy_to_mydate($date)."' -- date - DATE
         ,`from` = '$from' -- from - VARCHAR(255)
         ,from_region = '$from_region' -- from_region - VARCHAR(255)
         ,from_AOGUID = '$from_AOGUID' -- from_AOGUID - VARCHAR(255)
         ,from_house = '$from_house' -- from_house - VARCHAR(255)
         ,from_appart = '$from_appart' -- from_appart - VARCHAR(255)
         ,from_fio = '$from_fio' -- from_fio - VARCHAR(255)
         ,from_phone = '$from_phone' -- from_phone - VARCHAR(255)
         ,time_ready_from = '$time_ready_from' -- time_ready_from - TIME
         ,time_ready_end = '$time_ready_end' -- time_ready_end - TIME
         ,from_comment = '$from_comment' -- from_comment - VARCHAR(255)
         ,id_car = '$car_courier' -- id_car - INT(11)
         ,dk = NOW() -- dk - DATETIME
		WHERE id = '$order_id'
		";
		$this->query($sql);
	...
	public function update_routes($order_id,$params){
		if (is_array($params ['to'])) {
			...
			foreach ($params ['to'] as $key => $item) {
                ...
                $params ['target'][$key] = isset($params ['target'][$key]) ? $params ['target'][$key] : 0;
				$sql_values .= ($key > 0)?',':'';
                $sql_values .= ' (\''.$order_id.'\',\''.$params ['to'][$key].'\',\''.$params ['to_region'][$key].'\',\''.$params ['to_AOGUID'][$key].'\',
                            \''.$params ['to_house'][$key].'\',\''.$params ['to_appart'][$key].'\',\''.$params ['to_fio'][$key].'\',\''.$params ['to_phone'][$key].'\',
							\''.$params ['to_time'][$key].'\',\''.$params ['to_time_end'][$key].'\',\''.$params ['to_comment'][$key].'\',
							\''.$params ['cost_route'][$key].'\',\''.$params ['cost_tovar'][$key].'\',\''.$params ['cost_car'][$key].'\',
							\''.$params ['goods_type'][$key].'\',\''.$params ['goods_val'][$key].'\',\''.$params ['pay_type'][$key].'\',
							\''.$params ['status'][$key].'\',\''.$params ['target'][$key].'\', NOW()	)';
			}
			if ($sql_values != '') {
                $sql = "INSERT INTO orders_routes (id_order,`to`,`to_region`,`to_AOGUID`,`to_house`,`to_appart`,`to_fio`,`to_phone`,
                        `to_time`,`to_time_end`,`to_comment`,`cost_route`,`cost_tovar`,`cost_car`,goods_type,goods_val,`pay_type`,`id_status`,`target`,dk) 
                        VALUES $sql_values";
                $this->query($sql);
            }
		}
...
class ordersProcess extends module_process {
	...
	public function update($_action = false) {
		...
		if ($action == 'orderUpdate') {
            ...
            $params['from_AOGUID'] = $this->Vals->getVal('from_AOGUID', 'POST', 'array');
            $params['from_house'] = $this->Vals->getVal('from_house', 'POST', 'array');
            ...
            $params['time_ready_from'] = $this->Vals->getVal('time_ready_from', 'POST', 'array');
            $params['time_ready_end'] = $this->Vals->getVal('time_ready_end', 'POST', 'array');
            ...
            $params['target_time'] = $this->Vals->getVal('target_time', 'POST', 'array');
            $params['target'] = $this->Vals->getVal('target', 'POST', 'array');
            $params['to_comment'] = $this->Vals->getVal('to_comment', 'POST', 'array');
...
*/
	...
	public function getOrderTextInfo($order_id){
        ...
        $order_info_message .= "<b>Откуда:</b> " . $order_info['from_addr'] . "\r\n";
        $order_info_message .= "<b>Отправитель:</b> " . $order_info['from_fio'] . " [" . $order_info['from_phone'] . "]\r\n";
        $order_info_message .= "<b>Готовность:</b> " . $order_info['time_ready_from'] . ' - ' . $order_info['time_ready_end'] . "\r\n";
        ...
        foreach ($order_routes_info as $order_route_info) {
            ...
            $order_info_message .= ($order_route_info['to_comment'] != '')?" <i>" . $order_route_info['to_comment'] . "</i>\r\n":"";
